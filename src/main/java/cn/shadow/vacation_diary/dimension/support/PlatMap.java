...
//	public World world;
	...
	public PlatMap(VocationCityWorldGenerator generator, ShapeProvider shapeProvider, int originX, int originZ) {
		super();
...
//		this.world = generator.getWorld();
	...
	public double getNaturePercent() {
		return naturalPlats / (Width * Width) + generator.getWorldSettings().ruralnessLevel;
	}
	...
	public final void generateBiome(InitialBlocks chunk, LinearBiomeContainer biomes) {
		int platX = chunk.sectionX - originX;
		int platZ = chunk.sectionZ - originZ;

		PlatLot platlot = platLots[platX][platZ];
		if (platlot != null) {
			platlot.generateBiome(generator, this, chunk, biomes, context, platX, platZ);
		}
	}
	
	public final void decorate(InitialBlocks chunk, LinearBiomeContainer biomes) {
		int platX = chunk.sectionX - originX;
		int platZ = chunk.sectionZ - originZ;

		PlatLot platlot = platLots[platX][platZ];
		if (platlot != null) {
			platlot.decorate(generator, this, chunk, biomes, context, platX, platZ);
		}

	}

	public final void generateBlocks(RealBlocks chunk) {
...
//				generator.reportMessage(".....>>>>> planting nature");
	...
	public void paveLot(int x, int z, boolean roundaboutPart) {
		if (generator.getWorldSettings().inRoadRange(originX + x, originZ + z)
...
//				generator.reportMessage("Roundabout? " + generator.settings.includeRoundabouts + ", odds: " + context.oddsOfRoundAbouts + " context: " + context.toString());
				if (generator.getWorldSettings().includeRoundabouts && generator.getWorldSettings().inCityRange(originX + x, originZ + z)
